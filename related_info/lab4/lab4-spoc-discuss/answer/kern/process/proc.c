...
proc_run(struct proc_struct *proc) {
    ...
    if (proc != current) {
        ...
        //cprintf("qw... before switch:prev: %d state %d next:%d state %d\n",prev->pid,prev->state ,next->pid,next->state);
        local_intr_save(intr_flag);
		{

			/*
			if(current->pid == 2 && qwNum == 0)
			{
				qwNum = 1;
				int pid4= kernel_thread(init_main, "qw... init main4: Hello world!!", 0);
...
do_wait(int pid, int *code_store) {
	...
	//cprintf("qw...before do_wait: begin %d state %d\n",proc->pid, proc->state);
repeat:
...
proc_init(void) {
    ...
    int pid3= kernel_thread(init_main, "qw... init main3: Hello world!!", 0);
